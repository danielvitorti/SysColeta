...
class Etiqueta extends CI_Controller
{
    ...
    public function index() 
    {
        ...
        $total_row = $tempdb->from('etiqueta')->count_all_results();
        $this->db->limit($config['per_page'], $page);
        $this->db->order_by("Id", "desc");
        $result['etiquetas'] = $this->db->get('etiqueta')->result_array();
        $config['base_url'] = base_url('etiqueta')."?busca=$busca";
        ...
        $data['content'] = $this->load->view('etiqueta/index', $result, true);
    ...
    public function cadastro()
    {
	    ...
	    $data['content'] = $this->load->view('etiqueta/cadastro', $result, true); 
    ...
    public function salvar()
    {
        ...
        {   
            ...
            $this->Etiqueta_model->Descricao = $this->input->post('Descricao');

            
            if(is_null($Id) || empty($Id))
            {
                if($this->Etiqueta_model->inserir())
                {
            ...
            {               
                ...
                $this->Etiqueta_model->Id = $Id;
                if($this->Etiqueta_model->alterar())
                {
    ...
    public function alterar()
    {
        ...
        $this->db->where('Id', $Id);
        $etiqueta['etiqueta']= $this->db->get('etiqueta')->row_array();    

        if(is_null($etiqueta))
            redirect('etiqueta');

        $data['usuario'] = $this->session->userdata["usuario"];  
        $data['content'] = $this->load->view('etiqueta/alterar', $etiqueta, true);
        $data['nivelAcesso']   = (int)$this->session->userdata["nivelAcesso"];
    ...
    public function excluir()
    {
        ...
        else
        {
            ...
            if($this->Etiqueta_model->excluir())
            {
    ...
    public function pdf()
    {
        ...
        <div style="text-align: center; font-weight: bold;">
            <center><h2> Óleo Local - Coleta Seletiva - Etiquetas </h2></center>
        </div>');
        $mpdf->SetHTMLFooter('
        <table width="100%">
            <tr>
        ...
        if ($busca != '') 
        {
            $this->Etiqueta_model->Descricao = $busca;
        }

        $etiquetas = $this->Etiqueta_model->buscarPorDescricao()->result_array();
        ...
        $mpdf->WriteHTML($html);
        $pdf = 'Oleo-Local-Coleta-Seletiva-Etiquetas'.date('dmY').'.pdf';
    ...
    public function excel()
    {
        ...
        $arquivo = 'Oleo-Local-Coleta-Seletiva-Etiquetas'.date('dmY').'.xls';
